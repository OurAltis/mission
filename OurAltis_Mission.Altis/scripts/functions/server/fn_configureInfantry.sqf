...

{
	...
	} forEach (GVAR(Infantry) select _sideIndex select 1);
	
	if(_baseIndex == -1) then {
		// base has not yet been associated with any loadouts -> initialize
		_baseIndex = GVAR(Infantry) select _sideIndex select 1 pushBack [_base, []];
	};
	
	
	// add unit to respective side and base
	(((GVAR(Infantry) select _sideIndex) select 1 select _baseIndex) select 1) pushBack [_className, _amount];
