...
if (_object1 isKindOf "Man") then {
	if (side group _object1 isEqualTo civilian) then {
		...
		GVAR(voteActionReady) = _object1 addAction ["<t color='#3ADF00'>" + localize "OurA_str_voteOptionReady" + "</t>", {_this call FUNC(doVote)}, nil, 0, false, true, "", "!(" + QPGVAR(votingFinish) + ") && !(" + QGVAR(playerReady) + ")"];
		GVAR(voteActionNotReady) = _object1 addAction ["<t color='#3ADF00'>" + localize "OurA_str_voteOptionNotReady" + "</t>", {_this call FUNC(doVote)}, nil, 0, false, true, "", "!(" + QPGVAR(votingFinish) + ") && " + QGVAR(playerReady)];
	}
};
